...
class Process:
    ...
    def load_CIFAR10(self):
        ...
        Xte, Yte = self.load_CIFAR_batch(
            os.path.join(self.data_path, 'test_batch'))  # test data from testing pickle batches
        return Xtr, Ytr, Xte, Yte

    # generate h5 file
    def generate_h5(self, Xtr, ytr, Xtst, ytst, filename):
    ...
    def split_data_from_dirs(self, data, split):
        """
...
if __name__ == "__main__":
    ...
    p = Process(asl_dir, categories)
    data = p.data_from_dirs(asl_dir, categories)  # acquire all the data.
    x_train, y_train, x_test, y_test = p.split_data_from_dirs(data, 0.98)
    p.generate_h5(x_train, y_train, x_test, y_test, "alphabets.h5")
    sys.exit(1)
